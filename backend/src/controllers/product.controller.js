...
const importProducts = asyncHandler(async (req, res, next) => {
    try {
        const filePath = path.join(process.cwd(), "assets", "TechKey_dummy_data.csv");
        ...
        return res.json(
            new ApiResponse(200, `Successfully imported/updated ${filteredData.length} products`)
        );
    } catch (error) {
        console.error("Import error:", error.message);
        return next(new ApiError(500, "Error occurred while importing products", error.message));
    }
});
